...
describe("BUG 1 (fixed) — free/busy label consistent with the Pitch flat-load rule", () => {
  it("a person whose only work is a no-calls Pitch shows as free, not busy", async () => {
    ...
    const { rows } = await pool.query<{ id: string }>(
      `INSERT INTO project (pl_id, client, project_link, project_type, expert_pool, calls_n, goal_total, status)
       VALUES ($1, 'Client_PitchOnly', 'https://example.test/proj/pitchonly', 'Pitch', 'Global', 0, 8, 'matched') RETURNING id`,
      [fx.plAlpha]
    );
    await pool.query(
  ...
  it("the same Pitch's profiles count normally again once it converts (callsN > 0)", async () => {
    ...
    const { rows } = await pool.query<{ id: string }>(
      `INSERT INTO project (pl_id, client, project_link, project_type, expert_pool, calls_n, goal_total, status)
       VALUES ($1, 'Client_PitchOnly2', 'https://example.test/proj/pitchonly2', 'Pitch', 'Global', 0, 8, 'matched') RETURNING id`,
      [fx.plAlpha]
    );
    await pool.query(
      `INSERT INTO assignment (project_id, deliverer_id, goal, delivered, custom_goal, custom_delivered, stage)
